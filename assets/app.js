...
const demoWorks = [{id:'demo-tide-atlas',title:'潮汐指挥台',description:'把散乱的灵感和任务变成一张会呼吸的航海图。',author_name:'比特海盗',author_role:'船长',cover_url:'',link:'',journey_log:'我想做一个不催促人的任务面板。\n\n先让 AI 帮我把一天的碎片整理成航线，再用颜色留下潮汐的起伏。它不替你开船，只告诉你现在最值得往哪儿划。',tags:['网页','AI 应用'],created_at:'2026-07-12T10:30:00Z'}];
...
async function approvedWorks(){if(!db)return demoWorks;const {data,error}=await db.from('works').select('*').eq('status','approved').order('created_at',{ascending:false});if(error)throw error;return data}
async function initLanding(){const target=document.querySelector('#home-work-count');if(!target)return;try{let count=demoWorks.length;if(db){const {count:approvedCount,error}=await db.from('works').select('id',{count:'exact',head:true}).eq('status','approved');if(error)throw error;count=approvedCount||0}target.textContent=`已收录 ${count} 件作品 · 持续更新中`}catch(error){target.textContent='作品持续更新中'}}
function setMessage(el,message,error=false){el.textContent=message;el.classList.remove('receipt');el.classList.toggle('error',error)}
...
async function initHome(){const grid=document.querySelector('#works-grid'), filter=document.querySelector('#tag-filter'), empty=document.querySelector('#empty-state');let works=[];try{works=await approvedWorks()}catch(error){grid.innerHTML=`<p class="status-note">作品暂时无法靠岸：${escapeHtml(error.message)}</p>`;return}document.querySelector('#work-count').textContent=`${works.length} 件作品已靠岸`;const render=active=>{const list=active==='全部'?works:works.filter(item=>(item.tags||[]).includes(active));grid.innerHTML=list.map(card).join('');empty.classList.toggle('hidden',list.length>0)};filter.innerHTML=['全部',...tags].map((tag,index)=>`<button class="${index===0?'active':''}" data-tag="${tag}">${tag}</button>`).join('');filter.addEventListener('click',event=>{const button=event.target.closest('button');if(!button)return;filter.querySelectorAll('button').forEach(item=>item.classList.toggle('active',item===button));render(button.dataset.tag)});render('全部')}
async function initWork(){
  ...
  let work;
  try{
    if(!db)work=demoWorks.find(item=>item.id===id);
